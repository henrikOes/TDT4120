...
def compatibility_graph(donors, recipients, k):
    # Initialize the result list - one list per donor
    donor_edges = []
    ...
    for i in range(len(donors)):
        ...
        # Check compatibility with each recipient
        for j in range(len(recipients)):
            # Count matching attributes between donor i and recipient j
            matching_attributes = 0
            
            # Compare each attribute position
            for attr_idx in range(min(len(donors[i]), len(recipients[j]))):
                if donors[i][attr_idx] == recipients[j][attr_idx]:
                    matching_attributes += 1
            
            # If they have at least k matching attributes, they are compatible
            if matching_attributes >= k:
                compatible_recipients.append(j)
        
        donor_edges.append(compatible_recipients)
