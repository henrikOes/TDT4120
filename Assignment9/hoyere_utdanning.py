...
class HigherEdSolver:
    def initialize(self, institutions):
        #setter hver parent til seg selv
        self.parent = {}
        for inst in institutions:
            self.parent[inst] = inst


    #for å finne parent brukes find
    def parent_institution(self, institution):
        return self.find(institution)


    def fuse(self, institution1, institution2, new_institution):
        # Finn øverste institusjoner (roter) for hver
        root1 = self.find(institution1)
        root2 = self.find(institution2)

        # Lagre den nye institusjonen som en ny "rot"
        self.parent[new_institution] = new_institution

        # Koble de to gamle røttene til den nye institusjonen
        self.parent[root1] = new_institution
        self.parent[root2] = new_institution
        
    def find(self, inst):
        if self.parent[inst] != inst:
            self.parent[inst] = self.find(self.parent[inst])
        return self.parent[inst]
