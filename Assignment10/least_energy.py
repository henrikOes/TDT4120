...
def least_energy(reactions, start, goal, laws_of_thermodynamics=True):
    ...
    # Dijkstra-like algorithm to find least energy path, handles negative weights
    import heapq
    pq = [(0, start, [start])]  # (energy, current_substance, path)
    min_energy = {start: 0}
    goal_path = []
    min_goal_energy = float('inf')

    while pq:
        energy, substance, path = heapq.heappop(pq)

        if energy > min_energy.get(substance, float('inf')):
            continue

        if substance == goal:
            if energy < min_goal_energy:
                min_goal_energy = energy
                goal_path = path
            # Continue searching for even better paths

        for neighbor, cost in graph.get(substance, []):
            new_energy = energy + cost
            if new_energy < min_energy.get(neighbor, float('inf')):
                min_energy[neighbor] = new_energy
                heapq.heappush(pq, (new_energy, neighbor, path + [neighbor]))
