...
# et stÃ¸rre sett med 500 genererte instanser. For Ã¥ teste pÃ¥ det
...
# ta litt tid Ã¥ kjÃ¸re alle de 500 ekstra testene.
use_extra_tests = False


def sheet_cutting(w, h, p):
    
    #lager en tabell med h rader og w kolonner
    table = []
    for i in range(h):
        table.append([0] * w)
    ...
    for height in range(h):
        for width in range(w):
            #sjekker om det finnes en pris for arket
            if (width + 1, height + 1) in p: #+1 fordi prisene er 1-indeksert
                table[height][width] = p[(width + 1, height + 1)]
            
            for cut in range(1, width + 1): #+1 fordi vi skal inkludere hele bredden
                #sjekker om det er bedre å kutte arket vertikalt
                if table[height][cut - 1] + table[height][width - cut] > table[height][width]:
                    table[height][width] = table[height][cut - 1] + table[height][width - cut]
            
            for cut in range(1, height + 1): #+1 fordi vi skal inkludere hele høyden
                #sjekker om det er bedre å kutte arket horisontalt
                if table[cut - 1][width] + table[height - cut][width] > table[height][width]:
                    table[height][width] = table[cut - 1][width] + table[height - cut][width]
                    
    return table[h - 1][w - 1] #-1 fordi tabellen er 0-indeksert
